...
def model_pipeline():
    with wandb.init():
        ...
        model, discriminator, train_loader, valid_loader, unlabelled_loader, criterion, disc_loss_fn, optimizer, disc_optimizer, scaler, ct_augment = make(config)

        train(model, discriminator, train_loader, valid_loader, unlabelled_loader, optimizer, disc_optimizer, criterion, disc_loss_fn, scaler, device, config, ct_augment)

        test(model, discriminator, config, round(config.num_iters / len(train_loader))*len(train_loader))
...
def make(config):
    ...
    unlabelled_dataset = UnlabelledCustomDatasetTBP(content_images)

    # unlabelled_x = sorted(glob("C:/Users/Josh/Desktop/4YP/Processed_Data/val/image/*"))
    # unlabelled_dataset = UnlabelledCustomDatasetTBP(unlabelled_x)
    ...
    return model, discriminator, train_loader, valid_loader, unlabelled_loader, criterion, disc_loss_fn, optimizer, disc_optimizer, scaler, ct_augment
...
def train_one_epoch(model, discriminator, labeled_loader, unlabeled_loader,
                    optimizer, disc_optimizer, loss_fn, disc_loss_fn, scaler,
    ...
    for (x, y), u in zip(labeled_loader, unlabeled_loader):
        ...
        predicted_labels = (disc_probs > 0.5).float()  # Use a 0.5 threshold on probabilities
        ...
        combined_unet_loss = supervised_loss - alpha * disc_loss
    ...
    return total_loss / len(labeled_loader), sup_loss / len(labeled_loader), disc_loss / len(labeled_loader), discriminator_accuracy

def train(model, discriminator, train_loader, valid_loader, unlabeled_loader, optimizer, discriminator_optimizer, loss_fn, discriminator_loss_fn, scaler, device, config, ct_augment):
    #fmap_combinations_str = "_layers_" + "_".join(str(index) for index in config.fmap_combinations) + "_"
    data_save_path = f'C:/Users/Josh/Desktop/4YP/AdversarialOutputs/FMStyle/models/{config.training_set}_bc_{config.base_c}_'# + fmap_combinations_str
    ...
    try:
        for epoch in range(num_epochs):
            train_loss, sup_loss, disc_loss, disc_acc = train_one_epoch(model, discriminator, train_loader, unlabeled_loader, optimizer, discriminator_optimizer, loss_fn, discriminator_loss_fn, scaler, device, config, ct_augment, epoch)
            wandb.log({"Total Training Loss": train_loss, "Supervised Training Loss": sup_loss, "Discriminator Training Loss": disc_loss, "Discriminator Accuracy": disc_acc, "Iteration": epoch*(len(train_loader))})
            ...
            if epoch % config.validevery == 0:
                ...
                if epoch % 30 == 0 and epoch > 0:
                    test(model, discriminator, config, epoch*(len(train_loader)))
...
def test(model, discriminator, config, true_num_iters):
    ...
    for test_set in test_sets:
        ...
        wandb.log({
            ...
            ,
            f"Average Discriminator Prediction_{test_set}": avg_discriminator_prediction
        })
...
if __name__ == "__main__":
    ...
    parameters_dict = {
        ...
        'num_iters': {'value': 100000},
        'batch_size': {'value': 5},
        'learning_rate': {'value': 0.03},
        'validevery': {'value': 3},
        'final_alpha': {'value': 3},
        'batch_norm_sanitize': {'values': [True]},
        'input_dropout': {'values': [0.5]},
        'regularizing_dropout': {'values': [0.5]}
    }

    sweep_config['parameters'] = parameters_dict
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    sweep_id = wandb.sweep(sweep=sweep_config, project="AdversarialOUTPUTS_FMstyle")
