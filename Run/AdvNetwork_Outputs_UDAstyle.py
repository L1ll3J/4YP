...
def make(config):
    ...
    #UNLABELLED DATALOADERS GO HERE - changes depending on experiment
...
def train(model, discriminator, train_loader, valid_loader, unlabeled_loader, optimizer, discriminator_optimizer, loss_fn, discriminator_loss_fn, scaler, device, config, ct_augment):
    ...
    data_save_path = f'C:/Users/Josh/Desktop/4YP/AdversarialKamnitsas/TEST/models/{config.training_set}_bc_{config.base_c}_' + fmap_combinations_str
        ...
        for epoch in range(num_epochs):
            ...
            if epoch % config.validevery == 0:
                ...
                if epoch % 30 == 0 and epoch > 0:
                    test(model, config, epoch*(len(train_loader)))
...
if __name__ == "__main__":
    ...
    sweep_id = wandb.sweep(sweep=sweep_config, project="AdversarialOUTPUTSTEST_LiveAug_")
